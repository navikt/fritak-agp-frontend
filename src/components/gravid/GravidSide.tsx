...
import { BekreftCheckboksPanel, Feiloppsummering, Radio, RadioGruppe, SkjemaGruppe, Textarea } from 'nav-frontend-skjema';
...
const GravidSide = (props: GravidSideProps) => {
  ...
  const [fnrFeilmelding, setFnrFeilmelding] = useState<string>(!validated ? EMPTY : (validated && fnrValid) ? EMPTY : INVALID_FNR);
  ...
  const [tiltakBeskrivelseFeilmelding, setTiltakBeskrivelseFeilmelding] = useState<string>(!validated ? EMPTY : REQUIRED_INPUT);
          ...
          <Panel>
            <SkjemaGruppe legend="Arbeidssituasjon og miljø"
              ...
              tross for helsetilstanden hennes?">
                <Radio label={'Ja'} name="sitteplass" value={'ja'} defaultChecked={tilrettelegge === true}
                       onClick={() => {
                         setTilrettelegge(true)
                       }}/>
                <Radio label={'Nei'} name="sitteplass" value={'nei'} defaultChecked={tilrettelegge === false}
                       onClick={() => {
